...
def FE_AL (df, n_samples, spacing, pca, shuf):
    ...
    shuf:       (bool) choices are 'True' OR 'False', apply data shuffle
    ...
    for k in range (0,len(n_queries)):
        ...
        # Metrics
        rmse = np.sqrt(mean_squared_error(y_val, y_pred_kj))
        r2 = r2_score(y_val, y_pred_kj)
        metrics.append([n_query,rmse,r2])
        # save the model 
        joblib.dump(regressor, "./trained_models/model"+str(n_query)+".pkl")
        # load the model fto use it or the next iteration 
        regressor = joblib.load("./trained_models/model"+str(n_query)+".pkl")
    
    #=================================================================
    # GPR Training by Random sample
    metrics_rand = []
    for n_samples in n_queries:
        ...
        rmse = np.sqrt(mean_squared_error(y_val, y_pred_kj))
        r2 = r2_score(y_val, y_pred_kj)
        metrics_rand.append([n_samples,rmse,r2]) 
